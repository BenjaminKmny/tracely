    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#fafafa', fontFamily: '...', overflowX: 'hidden' }}>
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10, marginBottom: 14 }}>
              ...
              <MetricCard label="Cheapest ride" value={fmt(activeStats?.cheapestRide ?? 0)} tooltip="Lowest single fare — click to see details" onClick={cheapestRide ? () => setRideDetailModal('cheapest') : undefined} />
              <MetricCard label="Most expensive" value={fmt(activeStats?.mostExpensiveRide ?? 0)} tooltip="Highest single fare — click to see details" onClick={mostExpensiveRide ? () => setRideDetailModal('expensive') : undefined} />
            </div>
                ...
                {dynamicRoutes.length === 0 ? (
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                      ...
                      <div style={{ width: isMobile ? 50 : 80, background: '#f0f0f0', borderRadius: 3, height: 5, flexShrink: 0 }}>
            ...
            {/* Ride types */}
            <div style={{ background: 'white', border: '1px solid #eee', borderRadius: 14, padding: '16px 18px', marginBottom: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#111', marginBottom: 12 }}>Ride types</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {stats.rideTypes.map((t, i) => {
                  const total = stats.rideTypes.reduce((s, r) => s + r.count, 0)
                  const pct = Math.round(t.count / total * 100)
                  return (
                    <div key={i} style={{ background: i === 0 ? ACCENT_LIGHT : '#f5f5f5', border: `1px solid ${i === 0 ? ACCENT + '33' : '#eee'}`, borderRadius: 10, padding: '8px 12px' }}>
      ...
      {modalIndex !== null && modalLabel && (
        <>
          ...
          <div style={{ position: 'fixed', top: isMobile ? '10%' : '50%', left: isMobile ? '4%' : '50%', transform: isMobile ? 'none' : 'translate(-50%,-50%)', background: 'white', borderRadius: 16, width: isMobile ? '92vw' : '80vw', maxWidth: 900, maxHeight: '80vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', zIndex: 101, boxShadow: '0 24px 80px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid #f0f0f0' }}>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#111' }}>Rides · {modalLabel} {year}</div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ background: '#fafafa', borderBottom: '1px solid #f0f0f0' }}>
                    {(isMobile ? ['Date', 'Route', 'Fare'] : ['Date', 'Pickup', 'Dropoff', 'City', 'Type', 'Fare', 'Duration', 'Surge']).map(h => (
                ...
                <tbody>
                  {modalRides.length === 0 ? (
                    <tr><td colSpan={isMobile ? 3 : 8} style={{ padding: '32px', textAlign: 'center', color: '#aaa' }}>No rides for this period</td></tr>
                  ) : modalRides.map((r, i) => (
                    ...
                    >
                      {isMobile ? (
                        <>
                          <td style={{ padding: '10px 14px', color: '#555', whiteSpace: 'nowrap', fontSize: 12 }}>{new Date(r.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</td>
                          <td style={{ padding: '10px 14px', color: '#555', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 12 }}>{r.pickup?.split(',')[0] ?? '—'} → {r.dropoff?.split(',')[0] ?? '—'}</td>
                          <td style={{ padding: '10px 14px', color: '#111', fontWeight: 600, fontSize: 12 }}>{r.fare_eur > 0 ? `€${r.fare_eur.toFixed(2)}` : '—'}</td>
                        </>
                      ) : (
                        <>
                          <td style={{ padding: '10px 14px', color: '#555', whiteSpace: 'nowrap' }}>{formatDate(r.date)}</td>
                          <td style={{ padding: '10px 14px', color: '#555', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.pickup?.split(',')[0] ?? '—'}</td>
                          <td style={{ padding: '10px 14px', color: '#555', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.dropoff?.split(',')[0] ?? '—'}</td>
                          <td style={{ padding: '10px 14px', color: '#555' }}>{r.city || '—'}</td>
                          <td style={{ padding: '10px 14px', color: '#555' }}>{r.ride_type || '—'}</td>
                          <td style={{ padding: '10px 14px', color: '#111', fontWeight: 600 }}>{r.fare_eur > 0 ? `€${r.fare_eur.toFixed(2)}` : '—'}</td>
                          <td style={{ padding: '10px 14px', color: '#555' }}>{r.duration_mins > 0 ? `${r.duration_mins}m` : '—'}</td>
                          <td style={{ padding: '10px 14px' }}>
                            {r.surged ? <span style={{ background: '#FEF3C7', color: '#92400E', fontSize: 11, fontWeight: 600, padding: '2px 7px', borderRadius: 6 }}>Surge</span> : <span style={{ color: '#ccc' }}>—</span>}
                          </td>
                        </>
                      )}
                    </tr>
          ...
          <>
            ...
            <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: 'white', borderRadius: 16, width: isMobile ? '90vw' : 420, zIndex: 101, boxShadow: '0 24px 80px rgba(0,0,0,0.2)', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '20px 24px' }}>
                <div style={{ fontSize: 36, fontWeight: 900, color: ACCENT, letterSpacing: '-1px', marginBottom: 16 }}>€{ride.fare_eur.toFixed(2)}</div>
