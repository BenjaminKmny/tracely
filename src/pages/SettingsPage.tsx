    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#fafafa', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
        ...
        <div style={{ maxWidth: 560 }}>
          ...
          <div style={{ background: 'white', border: '1px solid #eee', borderRadius: 14, padding: '20px 24px', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 12, borderBottom: '1px solid #f5f5f5', marginBottom: 12 }}>
              ...
              <div style={{ fontSize: 13, color: '#111', fontWeight: 500 }}>{user?.email}</div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 12, borderBottom: '1px solid #f5f5f5', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ fontSize: 13, color: '#111' }}>
                {lastUpload
                  ? new Date(lastUpload).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })
                  : 'Never'}
              </div>
            </div>
          </div>
          ...
          <div style={{ background: 'white', border: '1px solid #FEE2E2', borderRadius: 14, padding: '20px 24px' }}>
            ...
            {/* Delete data */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 16, borderBottom: '1px solid #f5f5f5', marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 500, color: '#111', marginBottom: 2 }}>Delete all data</div>
                <div style={{ fontSize: 12, color: '#aaa' }}>Remove all your rides and orders. Your account stays active.</div>
              </div>
              <button
                onClick={() => setShowConfirm(true)}
                style={{ fontSize: 13, fontWeight: 600, color: '#EF4444', background: '#FEF2F2', border: '1px solid #FEE2E2', borderRadius: 8, padding: '7px 14px', cursor: 'pointer', whiteSpace: 'nowrap', marginLeft: 16 }}
              >
                Delete data
              </button>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ fontSize: 13, fontWeight: 600, color: 'white', background: '#EF4444', border: 'none', borderRadius: 8, padding: '7px 14px', cursor: 'pointer', whiteSpace: 'nowrap', marginLeft: 16 }}
              >
                Delete account
              </button>
            </div>
      ...
      {showConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'white', borderRadius: 16, padding: '28px 28px', maxWidth: 380, width: '90%', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ fontSize: 13, color: '#888', textAlign: 'center', lineHeight: 1.6, marginBottom: 24 }}>
              This will permanently remove {stats?.rides ?? 0} rides and {stats?.orders ?? 0} Eats orders from your account. Your account will remain active. This cannot be undone.
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                onClick={() => setShowConfirm(false)}
                style={{ flex: 1, padding: '10px 0', borderRadius: 10, border: '1px solid #eee', background: 'white', color: '#555', fontSize: 14, fontWeight: 500, cursor: 'pointer' }}
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleting}
                style={{ flex: 1, padding: '10px 0', borderRadius: 10, border: 'none', background: '#EF4444', color: 'white', fontSize: 14, fontWeight: 700, cursor: deleting ? 'not-allowed' : 'pointer', opacity: deleting ? 0.7 : 1 }}
              >
